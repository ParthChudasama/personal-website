...
function TiltCard({ project, index }: { project: FeaturedProject; index: number }) {
  ...
  const handleMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    e.currentTarget.style.transform = `perspective(1200px) rotateY(${x * 7}deg) rotateX(${-y * 7}deg) translateZ(8px)`;
    e.currentTarget.style.transition = 'transform 0.1s ease';
  };

  const handleLeave = (e: React.MouseEvent<HTMLDivElement>) => {
    e.currentTarget.style.transform = 'perspective(1200px) rotateY(0deg) rotateX(0deg) translateZ(0)';
    e.currentTarget.style.transition = 'transform 0.6s cubic-bezier(0.215,0.61,0.355,1)';
  };

  return (
    <div
      className={`featured-project ${isRight ? 'right' : ''}`}
      ...
      <div className="project-image">
        ...
        >
          <div className="project-image-placeholder">
            <span className="project-image-number">0{index + 1}</span>
          </div>
        </a>
      </div>
    </div>
